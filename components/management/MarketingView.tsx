...
import { ProductType, Language, ActiveCampaign } from '../../types';
import { MARKETING_CAMPAIGNS, TRANSLATIONS } from '../../constants';
import { TrendingUp, Zap, DollarSign, Clock, AlertTriangle, Tv, Smartphone, Calendar, Radio, Globe } from 'lucide-react';
...
const CAMPAIGN_ICONS: Record<string, any> = {
    'social_media': Smartphone,
    'influencer': Globe,
    'tv_ad': Tv,
    'esports_sponsor': Calendar,
    'tech_expo': Radio // Or any other suitable icon
};
            ...
            <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4">
                ...
                <div className="space-y-3">
                    {MARKETING_CAMPAIGNS.map(campaign => {
                        const nameKey = `${campaign.id}_name` as keyof typeof t;
                        const descKey = `${campaign.id}_desc` as keyof typeof t;
                        const Icon = CAMPAIGN_ICONS[campaign.type] || CAMPAIGN_ICONS[campaign.id] || Zap;

                        return (
                            <div key={campaign.id} className="bg-slate-900/50 border border-slate-700 rounded-lg p-4 relative overflow-hidden group hover:border-slate-600 transition-all">
                                {/* Background Icon */}
                                ...
                                <div className="flex justify-between items-start mb-3 relative z-10">
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-400 mt-1 max-w-[70%]">{t[descKey] || campaign.description}</p>
                                    </div>
                                    <div className="text-right">
                                        <div className="bg-slate-900 px-3 py-1 rounded-full text-emerald-400 font-mono font-bold text-sm border border-slate-800">
                                ...
                                <div className="grid grid-cols-2 gap-3 relative z-10">
                                    <Button
                                        ...
                                        className="text-xs font-bold uppercase bg-blue-600 hover:bg-blue-500 text-white border-none shadow-lg"
                                    >
                                        {t.launch} CPU
                                    </Button>
                                    <Button
                                        ...
                                        disabled={money < campaign.cost}
                                        className="text-xs font-bold uppercase bg-emerald-600 hover:bg-emerald-500 text-white border-none shadow-lg"
                                    >
                                        {t.launch} GPU
                                    </Button>
                                </div>
